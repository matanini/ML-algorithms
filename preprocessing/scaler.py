...
class Scaler:
    ...
    def __scale_minmax(self, X: pd.DataFrame) -> None:
        for col in X.columns:
            min_value = self.scale_data.loc["min", col]
            max_value = self.scale_data.loc["max", col]
    ...
    def __scale_t_distribution(self, X: pd.DataFrame) -> None:
        for col in X.columns:
            mean_value = self.scale_data.loc["mean", col]
            std_value = self.scale_data.loc["std", col]
