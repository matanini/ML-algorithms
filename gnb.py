...
class GNB:
    ...
    def fit(self, Xtrain: pd.DataFrame, ytrain: pd.Series) -> None:
        ...
        self.cls_priors = self._calc_class_priors(ytrain)
        self.mean_likelihood = self._calc_mean_for_likelihood(Xtrain, ytrain)
        self.std_likelihood = self._calc_std_for_likelihood(Xtrain, ytrain)

    def predict(self, xtest: pd.DataFrame) -> pd.Series:
        self.aposterior_probs = self._calc_aposterior_probs(xtest)
        return self.aposterior_probs.idxmax(axis=1)

    def _calc_class_priors(self, ytrain: pd.Series):
        lst_priors = []
    ...
    def _calc_mean_for_likelihood(self, xtrain: pd.DataFrame, ytrain: pd.Series):
    ...
    def _calc_std_for_likelihood(self, xtrain: pd.DataFrame, ytrain: pd.Series):
    ...
    def _calc_gaussian_pdf_prob(self, x_feature_val, feature_mean, feature_std):
        exponent = np.exp(
            -((x_feature_val - feature_mean) ** 2 / (2 * feature_std**2))
        )
        return (1 / ((2 * np.pi) ** (1 / 2) * feature_std)) * exponent

    def _calc_aposterior_probs(self, Xtest: pd.DataFrame):
        ...
        for row in Xtest.iterrows():
            for cls in self.cls_list:
                ...
                for j, feature in enumerate(row[1]):
                    calc_apos = calc_apos * self._calc_gaussian_pdf_prob(
                        feature,
